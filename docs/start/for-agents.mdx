...
10. **Use a browser pool when start-up latency matters.** A [pool](/docs/sessions/pools) keeps browsers warm with fixed options; `POST /v1/browser/pools/{poolId}/acquire` with `{ "waitMs": 10000 }` answers in under a second with `{ leaseId, session }`, and `session` is an ordinary session from then on. Release it with `POST /v1/browser/pools/{poolId}/release` `{ leaseId }` in the same `finally` where you would stop a session. Do not create pools on the person's behalf without asking: they hold warm browsers against an account-wide cap.
...
Useful extras: `GET /v1/browser/session?sessionId=<id>` for status and bandwidth, `GET /v1/browser/sessions?status=active` to list, `PATCH /v1/browser/session?sessionId=<id>` with `{ "note": "…" }` to label a run. Full options for the create body are on [Create a session](/docs/sessions/create#options). When a [pool](/docs/sessions/pools) exists, `POST /v1/browser/pools/{poolId}/acquire` replaces the create call and `POST /v1/browser/pools/{poolId}/release` the stop; pool ids go in the path.
...
- [ ] A browser taken from a pool is released (or stopped) in the same `finally`; no pool was created without asking.
