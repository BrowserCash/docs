...
- **Unattended jobs clean up after earlier runs.** A crash before `finally` leaves a session running until its `duration`. At start-up, `GET /v1/browser/sessions?status=active`, stop the ones your job created (match on `note`), then start the new one.
...
- Most tasks need no options: POST https://api.driver.dev/v1/browser/session with {} gives a
  hosted US Chrome and returns { sessionId, status, cdpUrl }. Use "country" for geo-specific
  sites, "profile" {name, persist:true} to keep logins, "duration" (60–3600 s, default 3600)
  to cap how long a forgotten session lives.
...
- Always stop the session in a finally: DELETE /v1/browser/session?sessionId=<id>.
  ...
  sessionId in logs.
- 402 (no credits) and 403 (limit): report. 503: back off, cap, report. 504 and 500: retry once.
...
export interface Session {
  ...
  note: string | null;
  bandwidthBytes?: number;     // on GET: 0 while running, the metered total afterwards
}
...
export const createSession = (opts: CreateOptions = {}) => call<Session>("POST", "/v1/browser/session", opts);
...
export async function stopSession(id: string): Promise<void> {
  ...
  if (!r.success) throw new DriverError(500, "stop did not succeed");
}

/** Create normally returns an active session; this covers the rare `starting` answer. */
async function waitForActive(s: Session, timeoutMs = 60_000): Promise<Session> {
...
export async function withBrowser<T>(
  opts: CreateOptions,
  fn: (page: Page, browser: Browser, session: Session) => Promise<T>,
): Promise<T> {
...
- [ ] 402 and 403 are reported to the user; 503 is backed off with a cap, never looped.
