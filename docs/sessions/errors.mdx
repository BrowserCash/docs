...
<Note>The dedicated-IP codes here (`invalid_spec`, `dedicated_ip_not_owned`, `no_dedicated_ip`, `dedicated_ip_unhealthy`) arrive with the next gateway release, alongside `dedicated://` in `proxyUrl`. The dashboard resolves the address before the create call, so Launch browser never returns them.</Note>
