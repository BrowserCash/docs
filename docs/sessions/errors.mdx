...
|---|---|---|
| `400` | Invalid or incompatible options, an unknown field on create, a missing `sessionId`, or an update to an ended session. The message names the problem. | No. Fix the request. |
...
| `403` | Concurrent-browser limit (`plan.concurrent_browsers` on `GET /v1/account/billing`; `starting` sessions count, a stop frees the slot as soon as it returns), or a feature (extensions) the account isn't enabled for. | Only after a session ends. Never in a loop. |
| `404` | Session, extension, pool, lease or account not found: unknown id, or one belonging to another workspace. Deleting an unknown profile isn't a 404; it answers `{ "success": true }`. | No. |
...
| `503` | No browser capacity. Carries `Retry-After: 2` and `"code": "browser_capacity_unavailable"`. Or the session service is unavailable. | Yes: back off with jitter, cap the attempts, then tell the user. |
