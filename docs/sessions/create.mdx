...
| `proxyUrl` | string | Driver's network | Your own `socks5://`, `socks5h://` or `tcperfect://` URL. `dedicated://…` ships with the next gateway release; until then reserve and use dedicated IPs from the dashboard. `dedicated://…` ships with the next gateway release; until then reserve and use dedicated IPs from the dashboard. `dedicated://…` ships with the next gateway release; until then reserve and use dedicated IPs from the dashboard. `dedicated://…` ships with the next gateway release; until then reserve and use dedicated IPs from the dashboard. See [Proxies](/docs/options/proxies). |
