...
| `proxyUrl` | string | Driver's network | Your own `socks5://` or `socks5h://` URL. `dedicated://…` names one of your [dedicated IPs](/docs/options/proxies#dedicated-ips), and is accepted from the dashboard today; API support ships with the next gateway release. See [Proxies](/docs/options/proxies). |
