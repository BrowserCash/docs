...
| `proxyUrl` | string | Driver's network | Your own `socks5://`, `socks5h://` or `tcperfect://` URL. `dedicated://…` is not accepted by the API yet; reserve and use dedicated IPs from the dashboard until the next gateway release. See [Proxies](/docs/options/proxies). |
