...
After a session ends its dashboard page keeps: status and duration, bandwidth and cost, node and country, network, how it was started (type, profile, note, API key), timestamps. Log the `sessionId` so you can find a run later.
