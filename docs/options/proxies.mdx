---
title: "Proxies"
description: "Sessions egress from Driver's ISP networks by default. proxyUrl routes through your own SOCKS5 endpoint, or one of your dedicated IPs."
---
...
<Note>
Reserving, renewing and releasing addresses works today, and so does using one for a session started from the
dashboard. Naming an address over this API — `proxyUrl: "dedicated://…"` and `GET /v1/network/ips` — ships with
the next gateway release; until then both answer `400` and `404`.
</Note>
...
`GET /v1/network/ips` lists your addresses with country, city, network type, label and status once the release
above lands. Until then, copy an address from the Network page.
