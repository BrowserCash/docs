...
A dedicated IP is a public address only your workspace's sessions use, so its reputation is yours alone. Reserve
...
subscription line; adding or releasing mid-month is prorated.
...
Release an address from the Network page; the subscription drops by one seat and the address goes back to the
pool. If a renewal payment fails, the addresses keep working for 7 days while the card is retried, then they're
released.
