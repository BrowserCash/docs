...
Driver provides a live viewer where you can watch and interact with your browser session in real-time.

To view a session, construct the viewer URL using your session's `cdpUrl`:

```
https://viewer.driver.dev?ws=<encoded-cdp-url>
```
...
const viewerUrl = `https://viewer.driver.dev?ws=${encodedWs}`;
...
viewer_url = f"https://viewer.driver.dev?ws={encoded_ws}"
...
```bash cURL
curl -X GET "https://api.driver.dev/v1/browser/session?sessionId=sess_abc123xyz" \
...
```bash cURL
curl -X GET "https://api.driver.dev/v1/browser/sessions?page=1&pageSize=20" \
  -H "Authorization: Bearer $DRIVER_API_KEY"
```
