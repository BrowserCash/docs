...
- Copy your Agent API key from the <a href="https://app.driver.dev">dashboard</a>.
...
```bash cURL
curl -X POST "https://api.driver.dev/v1/agent/task" \
...
DRIVER_API_KEY = os.getenv('DRIVER_API_KEY')

resp = requests.post(
    "https://api.driver.dev/v1/agent/task",
    headers={
        "Authorization": f"Bearer {DRIVER_API_KEY}",
...
An agent's state can either be `active`, `completed`, or `failed`. You can see the full output shape [here](https://api.driver.dev/scalar#get/v1agenttask).
...
```bash cURL
curl -H "Authorization: Bearer $DRIVER_API_KEY" \
  "https://api.driver.dev/v1/agent/task?taskId=$TASK_ID"
```

```python Python
import os, requests

API_BASE = os.getenv('API_BASE', 'https://api.driver.dev')
DRIVER_API_KEY = os.getenv('DRIVER_API_KEY')
TASK_ID = os.getenv('TASK_ID')

resp = requests.get(
    f"{API_BASE}/v1/agent/task",
    headers={"Authorization": f"Bearer {DRIVER_API_KEY}"},
...
This will return a paginated list of tasks you have created. It can have a slightly different shape than fetching a single task. You can see the full output shape [here](https://api.driver.dev/scalar#get/v1agenttasks).
...
```bash cURL
curl -H "Authorization: Bearer $DRIVER_API_KEY" \
  "https://api.driver.dev/v1/agent/tasks?pageSize=20&page=1"
```
